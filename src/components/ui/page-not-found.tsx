...
export default function NotFoundPage() {
  return (
    <div className="fixed inset-0 w-full h-screen bg-white dark:bg-gray-900 overflow-x-hidden flex justify-center items-center z-[9999]">
      <MessageDisplay />
      <CharactersAnimation />
      <CircleAnimation />
    </div>
...
function MessageDisplay() {
  ...
  return (
    <div className="absolute flex flex-col justify-center items-center w-[90%] h-[90%] z-50">
      <div 
        className={`flex flex-col items-center transition-opacity duration-500 ${
          isVisible ? 'opacity-100' : 'opacity-0'
        }`}
      >
        <div className="text-2xl sm:text-3xl md:text-4xl font-semibold text-gray-900 dark:text-white m-[1%] font-inter">
          Page Not Found
        </div>
        <div className="text-6xl sm:text-7xl md:text-8xl lg:text-9xl font-bold text-gray-900 dark:text-white m-[1%] font-playfair">
          404
        </div>
        <div className="text-sm sm:text-base md:text-lg w-full max-w-2xl text-center text-gray-600 dark:text-gray-400 m-[1%] font-inter px-4">
        ...
        <div className="flex flex-col sm:flex-row gap-4 sm:gap-6 mt-8">
          <Button
            onClick={() => router.back()}
            variant="outline"
            className="text-gray-900 dark:text-white border-2 border-gray-300 dark:border-gray-600 hover:border-slate-900 dark:hover:border-white hover:bg-slate-900 dark:hover:bg-white hover:text-white dark:hover:text-slate-900 transition-all duration-300 ease-in-out px-6 py-3 h-auto text-base font-semibold flex items-center gap-2 hover:scale-105 rounded-full font-inter"
          >
          ...
          <Button
            onClick={() => router.push("/")}
            className="bg-slate-900 hover:bg-blue-900 dark:bg-slate-800 dark:hover:bg-blue-900 text-white transition-all duration-300 ease-in-out px-6 py-3 h-auto text-base font-semibold flex items-center gap-2 hover:scale-105 rounded-full shadow-xl hover:shadow-2xl transform hover:scale-105 font-inter"
          >
...
function CharactersAnimation() {
  ...
  useEffect(() => {
    ...
    stickFigures.forEach((figure, index) => {
      ...
      // Make stick figures visible with filter for contrast
      stick.style.filter = 'drop-shadow(0 2px 4px rgba(0,0,0,0.3))';
  ...
  return (
    <div
      ref={charactersRef}
      className="absolute w-[99%] h-[95%] z-10 pointer-events-none"
    />
  );
}
...
function CircleAnimation() {
  ...
  const draw = () => {
    ...
    // Use dark color for light mode, light color for dark mode
    context.fillStyle = isDarkModeRef.current ? 'white' : '#1e293b'; // slate-800 for light mode
    ...
    // Continue animation indefinitely instead of stopping at 500
